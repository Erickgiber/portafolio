<script lang="ts">
  ...
  function scrollNext() {
    scrollToSection(Math.min(activeIndex + 1, total - 1));
  }
...
<style>
  ...
  .scroll-mouse:disabled {
    opacity: 0.4;
    cursor: default;
  }
  .scroll-mouse:hover:not(:disabled) {
  ...
  .arrow {
    ...
    width: 8px;
    height: 8px;
    border-left: 2px solid hsl(var(--foreground) / 0.65);
    border-bottom: 2px solid hsl(var(--foreground) / 0.65);
  ...
  :root[data-animations="off"] .section-progress .arrow {
    animation: none !important;
  }
  :root[data-animations="off"] .section-progress .bar-inner {
    transition: none !important;
  }
  :root[data-animations="off"] .section-progress .dot {
    transition: none !important;
  }
